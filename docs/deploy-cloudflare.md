...
4. **Save and Deploy** を押す前に、上の値が揃っていることを確認します。
   `OSAI_D1_DATABASE_ID` が無いとビルドは完了しますが収集OFFの設定が生成され、
   deployコマンドが「D1 IDを設定してやり直す」エラーで停止します（サイトは
   部分的に公開されません）。
...
2. **Settings → Variables and Secrets**（runtime変数）:
   ...
   `COLLECTION_NOTICE_VERSION=2026-09-21-v1`、`COLLECTION_CONTACT`（X案内）が
   表示されることを確認。これらは `wrangler.deploy.jsonc` 経由で適用され、
   dashboardで直接編集すると**次回deployで上書きされます**。変更はリポジトリ側
   （buildスクリプト）で行うのが正規の手順です。
   収集を一時停止したいときは `COLLECTION_ENABLED` を `false` にする変更を
   ビルド変数・リポジトリで行うか、一時的にdashboardで `false` にして
   （次回deployで戻る点に注意）運用します。
...
- [ ] 収集OFF時（`COLLECTION_ENABLED=false`）でも対局・解析・保存が続く。
- [ ] 翌日のCron後（日本時間12:17以降）、期限切れ行が削除されている
      （試験で作った期限切れデータで確認可。30日ちょうどの保証はなく、
      日次実行・Free枠・失敗時は翌回再試行の扱い）。
...
| deployが「D1 IDを設定して」で停止 | `OSAI_D1_DATABASE_ID` 未設定。手順2-3のIDをBuild変数へ設定しRetry |
